...
| DEF-1C-23 | Card authorization balance check (real, via baas-engine) | Stub always-sufficient in 1C | Phase 2 | Track-Card. **Phase 2 pitfall:** adding `@Transactional` to `AuthorizationDecisionService.decide` / `ReversalService.reverse` to make save+read atomic will open the Hibernate session BEFORE `PartnerContext` is set, routing to `public`. Must set context before the transaction opens (e.g. set context in a wrapper, then call a `@Transactional(REQUIRES_NEW)` inner bean). |
